                ...
                script {
                    catchError(buildResult: 'SUCCESS', stageResult: 'SUCCESS') {
                        sshagent (credentials: ['tf-key.']){
                            sh 'ssh -o StrictHostKeyChecking=no -i $JENKINS_HOME/tf.pem ubuntu@172.31.127.102 "docker ps -q | xargs docker stop"'
                        }
                    }
                    sshagent (credentials: ['tf-key.']){
                        sh "ssh -o StrictHostKeyChecking=no -i $JENKINS_HOME/tf.pem ubuntu@172.31.127.102"
